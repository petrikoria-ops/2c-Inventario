...
exports.parseSolicitudExcel = parseSolicitudExcel;
const norm = (v) => String(v !== null && v !== void 0 ? v : '').trim();
const normKey = (v) => norm(v).toUpperCase().normalize('NFD').replace(/[̀-ͯ]/g, '');
...
function parseFechaPlanilla(raw) {
...
function findHeaderRow(rows) {
    return rows.findIndex(r => r.some(c => normKey(c).startsWith('CODIGO')) &&
...
function parseSolicitudExcel(rows) {
    const result = { obra: '', supervisor: '', visitador: '', fechaEntrega: '', items: [] };
    ...
    for (const row of infoRows) {
        ...
        if (label.startsWith('OBRA'))
            result.obra = value;
        else if (label.startsWith('SUPERVISOR'))
            result.supervisor = value;
        else if (label.startsWith('VISITADOR'))
            result.visitador = value;
    ...
    for (const row of rows.slice(headerRowIdx + 1)) {
        ...
        if (descKey.includes('PENDIENTES SOLICITUDES ANTERIORES'))
            continue;
        if (descKey === 'DESCRIPCION')
            continue;
