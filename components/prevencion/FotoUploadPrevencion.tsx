...
import { subirFotoHallazgoPrevencion, getSignedUrlPrevencion } from '@/lib/supabase/storage'
...
export default function FotoUploadPrevencion({ inspeccionId, itemId, fotoUrl, disabled, onUploaded, onRemoved }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelado = false
    getSignedUrlPrevencion(getSupabaseBrowser(), fotoUrl).then(url => { if (!cancelado) setPreview(url) })
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true)
    try {
      const path = await subirFotoHallazgoPrevencion(getSupabaseBrowser(), inspeccionId, itemId, file)
      onUploaded(path)
