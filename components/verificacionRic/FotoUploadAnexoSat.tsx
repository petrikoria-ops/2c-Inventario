...
import { subirFotoAnexoSatTablero, getSignedUrl } from '@/lib/supabase/storage'
import { useToast } from '@/contexts/ToastContext'

interface Props {
  verificacionId: number
  tableroEntryId: number
  fotoUrl: string | null
  disabled?: boolean
...
export default function FotoUploadAnexoSat({ verificacionId, tableroEntryId, fotoUrl, disabled, onUploaded, onRemoved }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelado = false
    getSignedUrl(getSupabaseBrowser(), fotoUrl).then(url => { if (!cancelado) setPreview(url) })
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true)
    try {
      const path = await subirFotoAnexoSatTablero(getSupabaseBrowser(), verificacionId, tableroEntryId, file)
      onUploaded(path)
    ...
    <div className="flex items-center gap-2">
      ...
      {preview ? (
        <div className="relative">
          <img src={preview} alt="Foto del tablero" className="w-12 h-12 rounded-md object-cover border" style={{ borderColor: '#E2E4E7' }} />
          {!disabled && (
