...
// Auto-mapeo de headers del Excel → campos de la app
const ALIASES: Record<string, Record<ImportType | string, string>> = {
  materiales: {
  ...
  herramientas: {
    ...
    descripcion: 'descripcion', description: 'descripcion', nombre: 'descripcion',
    'marca/modelo': 'marca_modelo', marca_modelo: 'marca_modelo', marca: 'marca_modelo',
    modelo: 'marca_modelo',
    ...
    prox_mant: 'fecha_ultima_mant', fecha_mantencion: 'fecha_ultima_mant',
...
interface ReviewProps {
  ...
  approvedCorrIds: Record<string, boolean>   // corrId → true/false
  setApproved: (id: string, v: boolean) => void
  approveAll: () => void; rejectAll: () => void
  ...
  return (
    <div className="max-w-4xl space-y-4">
      ...
      <div className="flex gap-3 items-center pt-2">
        <button
          onClick={p.onConfirm}
          disabled={insertCount + updateCount === 0}
...
export default function ImportarMateriales() {
  ...
  // Parsear archivo con SheetJS (carga dinámica)
  ...
  // Aplicar mapeo → enviar al analyze endpoint
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      // Inicializar estado de correcciones (todas aprobadas por defecto)
      ...
      // Inicializar resoluciones de conflictos con defaults
      const initConflicts: Record<number, ConflictRes> = {}
      data.dbConflicts.forEach((c: any) => {
        initConflicts[c.rowIdx] = c.sameTable ? 'update' : 'skip'
      })
  ...
  const handleConfirm = useCallback(async () => {
    ...
    // Rebuild rows with approved corrections applied
    const mappedRows = rawRows.map((raw, idx) => {
  ...
  }, [analysis, rawRows, mapping, approvedCorrIds, conflictRes, fileDupRes, type, showToast])
