...
import { subirFotoAlimentador, getSignedUrlAlimentador } from '@/lib/supabase/storage'
...
export default function FotoUploadAlimentador({ pruebaId, itemId, fotoUrl, disabled, onUploaded, onRemoved }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelado = false
    getSignedUrlAlimentador(getSupabaseBrowser(), fotoUrl).then(url => { if (!cancelado) setPreview(url) })
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true)
    try {
      const path = await subirFotoAlimentador(getSupabaseBrowser(), pruebaId, itemId, file)
      onUploaded(path)
